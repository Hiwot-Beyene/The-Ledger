...
class EventSimulator:
    ...
    def _step_decision(self, target_state: str):
        ...
        self._emit(app,DecisionGenerated(
            application_id=self.application_id,orchestrator_session_id=sid,recommendation=rec,confidence_score=conf,
            ...
            key_risks=[] if rec=="APPROVE" else ["Revenue trend requires monitoring"],contributing_agent_sessions=[],model_versions={"orchestrator":MODEL},generated_at=self.t,
        ))
        if rec=="APPROVE":
